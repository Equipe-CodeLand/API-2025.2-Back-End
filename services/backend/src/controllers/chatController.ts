...
async function enviarMensagem(req: Request, res: Response) {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erro ao gerar relatório" + err.message });
  }
}
...
async function criarChat(req: Request, res: Response) {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erro ao criar chat: " + err.message });
  }
}
...
async function excluirChat(req: Request, res: Response) {
  try {
    const usuarioId = req.user?.id;
    ...
    // Verifica se o usuário é dono do chat
    const [verifica]: any = await db.query(
      "SELECT 1 FROM chat_usuario WHERE chat_id = ? AND usuario_id = ?",
      [chatId, usuarioId],
    );
    ...
    await db.query("DELETE FROM chat WHERE id = ?", [chatId]);
